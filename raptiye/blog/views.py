...
def get_post(request, year, month, day, slug):
	...
	captcha = None
	extra_context = {
		"allow_anonymous_comments": IS_COMMENTS_INSTALLED and settings.ALLOW_ANONYMOUS_COMMENTS and not request.user.is_authenticated(),
		"form": CommentForm(),
	}

	# don't create captcha if the user is not authenticated
	if IS_COMMENTS_INSTALLED and settings.ALLOW_ANONYMOUS_COMMENTS or request.user.is_authenticated():
